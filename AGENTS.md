...
每个交易日（默认周一至周五 HKT 16:30）执行每日扫描。**扫描时间可由用户自定义**（见下方 Scan Schedule）。
...
## Scan Schedule（用户可自定义）

用户可通过指令调整每日扫描时间：

- `设置扫描时间 09:30` 或 `设置扫描时间到 10:00` → 修改扫描触发时间
- `设置扫描日 1-5` → 周一至周五；`设置扫描日 1,2,3,4,5` → 同上
- `查看扫描时间` → 显示当前设置

操作方式：调用 `exec` 运行 `python3 -m src.cli schedule --time "09:30"` 来修改。
配置保存在 `scan_schedule.json` 并自动同步到 `cron/jobs.json`。
